...
class AttendanceProcessor:
    ...
    def _process_department(self, dept_name: str, dept_group: pd.DataFrame,
        ...
        """處理部門資料"""
        self.logger.info(f"🏢 處理部門: {dept_name}")
    ...
    def _generate_employee_report(self, emp_id: str, group: pd.DataFrame,
        ...
        self.logger.info(f"💾 生成 {emp_id}-{group.iloc[0]['Name']} 員工報表...")
    ...
    def _generate_master_report(self, emp_id: str, group: pd.DataFrame, wb_master):
        """生成主報表"""
        self.logger.info(f"💾 生成 {emp_id}-{group.iloc[0]['Name']} 主報表...")
        ...
        if emp_info_row.empty:
            self.logger.warning(f"⚠️ 員工ID: {emp_id} 在 masterdata.xlsx --> Employee 工作表中找不到")
            return
    ...
    def _show_completion_message(self):
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showinfo("完成通知", "報表已成功生成至 output 資料夾！")
